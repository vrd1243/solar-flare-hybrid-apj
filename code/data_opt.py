...
class sunspotImageDataSetBr(torch.utils.data.Dataset):
    ...
    def __init__(self, df, root_dir, transform, processed_root_dir='/srv/data/hdf5_processed/'):
    ...
    def __getitem__(self, idx):
        ...
        try:   
            filepath = os.path.join(self.processed_root_dir, filename)
            image = np.array(h5py.File(filepath, 'r')['hmi'])
            #print("Try to fetch", filepath)
        except:
            filepath_processed = os.path.join(self.processed_root_dir, filename)
            if os.path.exists(filepath_processed):
                os.remove(filepath_processed)
            ...
            image = cv2.resize(image,(128,128))
            try:
                h5f = h5py.File(os.path.join(self.processed_root_dir, self.name_frame.iloc[idx]), 'w')
                h5f.create_dataset('hmi', data=image)
                h5f.close()
            except Exception:
                pass
...
def generateTrainValidData(df, root_dir, splitType = 'by_harpnum'):
    ...
    # Split by harpnum: Train 70%, Valid 10%, Test 20%
        ...
        split_valid = int(0.8*harpnum_set.shape[0])
